...
class Customer:
    def __init__(self, no, x, y, demand, ready_time, due_date, service_time):
        self.no = no #numer klienta
        self.x = x #wspolrzedne
        self.y = y
        self.demand = demand #zapotrzebowanie na towar
        self.ready_time = ready_time #najwczesniejszy
        self.due_date = due_date #najpozniejszy
        self.service_time = service_time #czas obslugi klienta

def load_data(filename="RC101.txt"):
    customers = []
...
class Particle:
    def __init__(self, num_customers, vehicle_capacity, lambda_penalty, inertia, c1, c2):
        ...
        self.best_position = self.position[:] #najlepsza dotad pozycja
        self.best_fitness = float('inf') #najlepsza wartosc f. celu
        self.vehicles_used = 0 #liczba uzytych pojazdow
        self.routes = [] #trasy pojazdow
        self.vehicle_capacity = vehicle_capacity #poj
        self.lambda_penalty = lambda_penalty #kara za uzycie wiekszej liczby pojazdow
        self.inertia = inertia #inercja PSO - utrzymanie obecnej predkosci
        self.c1 = c1 # kongitywny - wlasna poz
        self.c2 = c2 #socjalny - globalna poz
        self.fitness = self.evaluate() #liczenie f. celu

"""
f. celu = laczna odleglosc + kara za liczbe pojazdow
1. zaczynamy w magazynie (klient 0)
2. przechodzimy przez klientow zgodnie z kolejnoscia self.position
3.spr czy mozna dodac klienta do obecnej trasy
- if przekracza czas okna lub pojemnosc -> robimy nowa trase
- if za wczesnie -> czekamy
4. jesli kleint nie moze byc obsluzony zwracamy inf (niewazne rozwiazanie)
"""
#ocena jakosci
    def evaluate(self):
        total_distance = 0
...
for param1, param2 in paired_experiments:
    ...
    for value1 in experiment_values[param1]:
        for value2 in experiment_values[param2]:
            ...
            results.append((param1, value1, param2, value2, best_fitness, global_best_particle.vehicles_used))
            values_x.append(len(values_x))
            values_y.append(best_fitness)
            labels.append(f"{param1}={value1},\n{param2}={value2}")

    bars = plt.bar(values_x, values_y, color='blue')

    # Dodanie wartości nad słupkami jako liczby całkowite
    for bar in bars:
        yval = bar.get_height()
        plt.text(bar.get_x() + bar.get_width()/2, yval, int(round(yval)), ha='center', va='bottom', fontsize=10)


    plt.xticks(values_x, labels, rotation=45, ha="right")
    plt.xlabel('Kombinacja parametrów')
    plt.ylabel('Najlepsza odległość')
    plt.title(f'Wpływ {param1} i {param2} na konwergencję')
    plt.tight_layout()  # Dopasowanie układu, aby tekst się mieścił

    # Zapis wykresu do pliku
    filename = f"Figure_{figure_counter}.png"
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    figure_counter += 1  # Inkrementacja licznika wykresów
...
df_results.to_csv("wyniki_experymentu.csv", index=False, encoding="utf-8-sig")

# Zapisanie wyników do pliku Excela
df_results.to_excel("wyniki_experymentu.xlsx", index=False, engine='openpyxl')
